...
const relativePath = getNestedObject(angular, [
  "projects",
  defaultProject,
  "architect",
  "build",
  "options",
  "outputPath",
]); // to identify relative build path when angular make build
...
fs.readdir(__dirname + buildPath, (err, files) => {
  files.forEach((file) => {
    if (
      file.match(
        /^(es2015-polyfills|main|polyfills|runtime|scripts|styles)+([a-z0–9.\-])*(js|css)$/g
      )
    ) {
      // regex is identified by build files generated
