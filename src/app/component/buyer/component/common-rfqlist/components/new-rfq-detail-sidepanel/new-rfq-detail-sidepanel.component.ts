...
@Component({
  selector: 'new-app-supplier-rfq-detail-sidepanel',
  templateUrl: './new-rfq-detail-sidepanel.component.html',
  styleUrls: ['./new-rfq-detail-sidepanel.component.scss']
})
export class NewSupplierRfqDetailSidepanelComponent implements OnInit, OnDestroy {
  ...
  closePartDrawer() {
    this.parentEmitter.emit({showSidePanel:false});
  }
  ngOnDestroy() {
    this.closePartDrawer();
  }
  ...
  getRfqDetails() {
    ...
    this._rfqService.getRFQExtraDetails(this.selectedRFQ.rfqId, supplierContactId, '').subscribe(
      result => {
        this.isLoader=false;
  ...
  getPartsList() {
    this._rfqService.getReviewRfqParts(this.selectedRFQ.rfqId, this.loggedId, false).subscribe(
      result => {
        if(result){
  ...
  GetSupplierRFQQuoteDetails() {
    ...
    this._SupplierService.GetSupplierRFQQuoteDetails(this.selectedRFQ.rfqId, this.loggedId, false).subscribe(
      result => {
  ...
  goToRfqDdetails(id) {
    ...
    this._rfqService.set(this.selectedRFQ.isRfqLike, 'isRfqLike');
    this._rfqService.set(this.selectedRFQ.isRfqDisLike, 'isRfqDisLike');
