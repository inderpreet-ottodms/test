...
const URL = "";
@Component({
  selector: "app-award-order-managment",
  templateUrl: "./award-order-mangment.component.html",
  styleUrls: ["./award-order-mangment.component.scss"],
  providers: [ConfirmationService],
})
export class AwardOrderManagmentComponent implements OnInit, DoCheck {
  ...
  getPartId() {
    this._rfqService.getRfqParts(this.rfqId).subscribe((result) => {
  ...
  getQuantity(data) {
    ...
    this._rfqService
      .getAwardingModelQuoteQuantityData(this.partsRequestViewModel)
      .subscribe((response) => {
        this.supplierQuantityData = response.data;
        this.rating = this.supplierQuantityData.noOfStars;
        this.supplierQuantityDropdown =
          this.supplierQuantityData.rfqQuoteItemList;
        // console.log("supplierQuantityDropdown",this.supplierQuantityDropdown)
        this.NPartQuantityColl = [];
        this.supplierQuantityDropdown.forEach((element) => {
          // console.log(" element.awardedQty.toString() + ' parts @ $' + element.totalPrice", element.awardedQty.toString() + ' parts @ $' + element.totalPrice)
          this.NPartQuantity.quantityDetails =
            element.awardedQty.toString() + " parts @ $" + element.totalPrice;
          // console.log("this.NPartQuantity.quantityDetails",this.NPartQuantity.quantityDetails)
          ...
          if (this.supplierQuantityDropdown.length === 1) {
            this.NPartArray.rfqQuoteItemsId =
              this.supplierQuantityDropdown[0].rfqQuoteItemsId;
          }
          this.initNPartQuantity();
        });
  ...
  makeQuantitySelection(manId, selectRfqQuoteItemsId, rfqpartid) {
    const localData = this.NPartArrayColl.find(
      (m) => m.manId === manId && m.rfqPartId === rfqpartid
    );
  ...
  setSupplierId(id) {
    if (id && id !== "undefined") {
      ...
      this.dropDownID = id;
      const ratingData = this.manufactureData.find(
  ...
  getSupplierList(rfqPartId) {
    return new Promise((resolve, reject) => {
      this._rfqService
        .getSupplierList(this.rfqId, rfqPartId)
        .subscribe((response) => {
          if (!response.isError) {
  ...
  getAwardingModelQuoteQuantityData(manufId, PartData, index) {
   let data = this.awardingModelQuoteDetailsViewModel[index].supplierList.find(
      (x) => x.id == manufId
    );
    let isRfqStatus;
    if (data) {
    ...
    if (
      this.awardingModelQuoteDetailsViewModel[index].rfqQuoteItemList.length !=
      0
    ) {
      ...
      if (manufId != null && manufId != "null" && isRfqStatus == false) {
        ...
        this.awardingModelQuoteDetailsRequestViewModel =
          new AwardingModelQuoteDetailsRequestViewModel();
        this.awardingModelQuoteDetailsRequestViewModel.rfqId = this.rfqId;
        this.awardingModelQuoteDetailsRequestViewModel.rfqPartId =
          PartData.rfqPartId;
        ...
        this._rfqService
          ...
          .subscribe(
            (res) => {
              if (res["result"] == true) {
                if (
                  this.awardingModelQuoteDetailsViewModel[index]
                    .rfqQuoteItemList != undefined &&
                  this.awardingModelQuoteDetailsViewModel[index]
                    .rfqQuoteItemList.length != 0 &&
                  this.awardingModelQuoteDetailsViewModel[index]
                    .rfqQuoteItemList != null
                ) {
                  this.awardingModelQuoteDetailsViewModel[
                    index
                  ].rfqQuoteItemList.forEach((item) => {
                    if (
                      this.awardRequestModel.rfqQuoteItemList != undefined &&
                      this.awardRequestModel.rfqQuoteItemList != null
                    ) {
                      if (this.awardRequestModel.rfqQuoteItemList.length != 0) {
                        this.awardRequestModel.rfqQuoteItemList.forEach(
                          (x, index) => {
                            this.rfqItemID.push(item.rfqQuoteItemsId);
                            if (x.rfqQuoteItemsId == item.rfqQuoteItemsId) {
                              this.awardRequestModel.rfqQuoteItemList.splice(
                                index,
                                1
                              );
                            }
                ...
                if (
                  ...
                  if (this.awardRequestModel.rfqQuoteItemList.length != 0) {
                    let nonMfgIndex =
                      this.awardRequestModel.rfqQuoteItemList.findIndex(
                        (x) =>
                          x.rfqPartStatusId ==
                          this.awardingModelQuoteDetailsViewModel[index]
                            .manufacturerId
                ...
                this.awardingModelQuoteDetailsViewModel[
                  index
                ].rfqQuoteItemList = res.data.rfqQuoteItemList;
                this.awardingModelQuoteDetailsViewModel[index].noOfStars =
                  res.data.noOfStars;
                this.awardingModelQuoteDetailsViewModel[index].manufacturerId =
                  manufId;
              }    
            },
            (error) => {
        ...
        let nonMfgIndex = this.awardRequestModel.rfqQuoteItemList.findIndex(
          (x) =>
            x.rfqPartStatusId ==
            this.awardingModelQuoteDetailsViewModel[index].manufacturerId
        );
        if (nonMfgIndex != -1) {
          ...
          if (
            ...
            this.awardingModelQuoteDetailsViewModel[
              ...
              if (this.awardRequestModel.rfqQuoteItemList.length != 0) {
                let i = this.awardRequestModel.rfqQuoteItemList.findIndex(
                  (x) => x.rfqQuoteItemsId == item.rfqQuoteItemsId
                );
                if (i != -1) {
        ...
        if (manufId == 18) {
          ...
          this.AwardPartQuatityRequestModel =
            new AwardPartQuatityRequestModel();
          this.AwardPartQuatityRequestModel.rfqQuoteItemsId = null;
          this.AwardPartQuatityRequestModel.rfqPartStatusId =
            this.awardingModelQuoteDetailsViewModel[index].manufacturerId;
          this.AwardPartQuatityRequestModel.unit =
            this.awardingModelQuoteDetailsViewModel[index].unit;
          this.AwardPartQuatityRequestModel.price =
            this.awardingModelQuoteDetailsViewModel[index].price;
          this.AwardPartQuatityRequestModel.isRfqStatus = true;
          this.AwardPartQuatityRequestModel.rfqPartId =
            this.awardingModelQuoteDetailsViewModel[index].rfqPartId;
          this.AwardPartQuatityRequestModel.awardedUnitTypeId =
            this.awardingModelQuoteDetailsViewModel[index].awardedUnitTypeId;
          this.awardRequestModel.rfqQuoteItemList.push(
            this.AwardPartQuatityRequestModel
          );
        } else {
          this.AwardPartQuatityRequestModel =
  ...
  onSelectQuantity(partId, rfqQuoteItemsId, index) {
    ...
    if (rfqQuoteItemsId === "null") {
      ...
      this.awardingModelQuoteDetailsViewModel[index].price = null;
      this.nextBtn = false;
      var newcheckRfqPartId = this.finalQuantityData.findIndex(
        (x) => x.rfqPartId == partId
      );
      ...
      let checkRfqPartId = this.finalQuantityData.findIndex(
        (x) => x.rfqPartId == partId
      );
      if (checkRfqPartId > 0) {
      ...
      if (
        ...
        this.awardingModelQuoteDetailsViewModel[index].rfqQuoteItemList.forEach(
          (item) => {
            if (
              item.rfqQuoteItemsId == rfqQuoteItemsId &&
              rfqQuoteItemsId != null &&
              rfqQuoteItemsId != "null" &&
              ...
              this.AwardPartQuatityRequestModel =
                new AwardPartQuatityRequestModel();
              this.AwardPartQuatityRequestModel.rfqQuoteItemsId =
                rfqQuoteItemsId;
              ...
              if (
                (rfqQuoteItemsId == null || rfqQuoteItemsId == "null") &&
                this.awardRequestModel.rfqQuoteItemList.length != 0
              ) {
      ...
      this.nextBtn = false;
      if (this.enableNextBtn && !this.isReqoute) {
        this.nextBtn = true;
      } 
    }
  }
  ...
  getUnit() {
    this._masterService.getQuantityUnit().subscribe((data: IQuantityUnit[]) => {
      this.iQuantityUnitColl = data;
    });
  }
  getQuantityUnit(key, quantity) {
    if (this.activeStatusFilterBtn === "Awarded") {
      const data = this.iRFQPartQuantityQuoteColl.find(
        (m) => m.rfqPartIdString === key && m.quantityLevel === quantity
      );
      if (data !== undefined) {
        if (data.isAwrded) {
    ...
    if (
      ...
      if (isSupplierNotExist) {
       ...
       if (this.selectedId < 21) {
        ...
        this.router.navigate(["/rfq/rfqdetail"], {
          queryParams: { rfqId: this.deCryptRFQIDUrl, rfq: "RFQ" },
        });
      } else {
        this.awardStatus = "Awarded";
        this.router.navigate(["/rfq/rfqdetail"], {
          queryParams: { rfqId: this.deCryptRFQIDUrl, order: "Order" },
        });
      }
      // ----------------------------------------------------------------------
     ...
     this.encrypted = EncryptId.toString();
      ...
      if (this.purchaseOrder == "") {
        ...
        this._rfqService.saveAwardData(data).subscribe((response) => {
  ...
  getRFQItemsId() {
    ...
    this._rfqService
      .GetRFQPartQuantityQuoteBYRfqId(iRFQPartQuantityQuote1)
      .subscribe((result: any) => {
  ...
  warning() {
    this._rfqService
      .getWarningSetting(this.selectedId)
      .subscribe((result: any) => {
  ...
  unitCheck(index, value, parId) {
    let newcheckRfqPartId = this.finalQuantityData.findIndex(
      (x) => x.rfqPartId == parId
    );
    if (newcheckRfqPartId > 0) {
      this.onlyForSupplierData = this.finalQuantityData.find(
        (x) => x.rfqPartId == parId
      );
  ...
  setFlowName() {
    let logMessage = "Rfq detail award modal 1 - open";
    this._rfqService
      .setAwardFlow(this.rfqId, logMessage)
      .subscribe((response) => {
        // this._toastr.success("PO Sent to Manufacturer", 'Success!');
      }),
      (error) => {};
  ...
  setFlowNameOnNext() {
    let logMessage = "Rfq detail award modal 2 - open";
    this._rfqService
      .setAwardFlow(this.rfqId, logMessage)
      .subscribe((response) => {
        // this._toastr.success("PO Sent to Manufacturer", 'Success!');
  ...
  setFlowNameUpload() {
    let logMessage = "Rfq detail award modal 2 - File Uploaded";
    this._rfqService
      .setAwardFlow(this.rfqId, logMessage)
      .subscribe((response) => {
        // this._toastr.success("PO Sent to Manufacturer", 'Success!');
